...
router.get('/CHECK-GASHMVKNG001', async (req, res) => {

  return res.json(GASHMVKNG001db['PO']);
});


router.post('/GASHMVKNG001db', async (req, res) => {
  //-------------------------------------
  // console.log('--GASHMVKNG001db--');
  // console.log(req.body);
  //-------------------------------------
  let finddb = [{}];GASHMVKNG001db
  try {

    finddb = GASHMVKNG001db;
    finddbbuffer = finddb;
    console.log(finddb);
...
router.post('/GETINtoGASHMVKNG001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoGASHMVKNG001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  check = GASHMVKNG001db;
...
router.post('/GASHMVKNG001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < GASHMVKNG001db['ItemPickcode'].length; i++) {
    if (GASHMVKNG001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = GASHMVKNG001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    GASHMVKNG001db['inspectionItem'] = ITEMSS;
    GASHMVKNG001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': GASHMVKNG001db["PO"], 'CP': GASHMVKNG001db["CP"], 'ITEMs': GASHMVKNG001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": GASHMVKNG001db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            GASHMVKNG001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            GASHMVKNG001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            GASHMVKNG001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              GASHMVKNG001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              GASHMVKNG001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          GASHMVKNG001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          GASHMVKNG001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          GASHMVKNG001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          GASHMVKNG001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:17180/GASHMVKNG001-feedback',
              { json: { "PO": GASHMVKNG001db['PO'], "ITEMs": GASHMVKNG001db['inspectionItem'] } },
    ...
    GASHMVKNG001db["POINTs"] = '',
      GASHMVKNG001db["PCS"] = '',
      GASHMVKNG001db["PCSleft"] = '',
      GASHMVKNG001db["UNIT"] = "",
      GASHMVKNG001db["INTERSEC"] = "",
      output = 'NOK';
  }
...
router.post('/GASHMVKNG001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    GASHMVKNG001db['GAPnameListdata'] = graph[0];//confirmdata
    GASHMVKNG001db['GAP'] = GASHMVKNG001db['GAPnameListdata'][`GT${GASHMVKNG001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/GASHMVKNG001-preview', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMVKNG001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = GASHMVKNG001db['preview'][0]

    if (GASHMVKNG001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = GASHMVKNG001db['preview'][0]

      pushdata['V5'] = GASHMVKNG001db['GAP'];
      pushdata['V1'] = `${GASHMVKNG001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      GASHMVKNG001db['confirmdata'].push(pushdata);
      GASHMVKNG001db['preview'] = [];
      output = 'OK';
      GASHMVKNG001db['GAP'] = GASHMVKNG001db['GAPnameListdata'][`GT${GASHMVKNG001db['confirmdata'].length + 1}`]

    } else if (GASHMVKNG001db['RESULTFORMAT'] === 'Number') {

      let pushdata = GASHMVKNG001db['preview'][0]

      pushdata['V5'] = GASHMVKNG001db['confirmdata'].length + 1
      pushdata['V1'] = `${GASHMVKNG001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/GASHMVKNG001-confirmdata-set', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001-confirmdata-set--');
  console.log(req.body);
  let input = req.body;
  GASHMVKNG001db['confirmdata'] = input;
  //-------------------------------------
  res.json('ok');
});

router.post('/GASHMVKNG001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      GASHMVKNG001db["PCSleft"] = `${parseInt(GASHMVKNG001db["PCS"]) - oblist.length}`;
      if (GASHMVKNG001db['RESULTFORMAT'] === 'Number' || GASHMVKNG001db['RESULTFORMAT'] === 'Text' || GASHMVKNG001db['RESULTFORMAT'] === 'Graph') {
        ...
        GASHMVKNG001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        GASHMVKNG001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(GASHMVKNG001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (GASHMVKNG001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (GASHMVKNG001db['INTERSEC'] !== '') {
                core = parseFloat(GASHMVKNG001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                GASHMVKNG001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (GASHMVKNG001db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
              ...
              let dataCheck = await axios.post("http://localhost:17180/GRAPH-recal", {
                "PO": GASHMVKNG001db["PO"],
                "MODE": "CDE",
                "ITEMs": "ITEMs-5f19aaa2fe12be0020dbd3c2",
                "NAME_INS": "GAS-HMVKNG-001",
                "INTERSEC": ""
              })
            }
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:17180/JUDEMENT", { "PO": GASHMVKNG001db["PO"], "CP": GASHMVKNG001db["CP"] })
      ...
      GASHMVKNG001db["ITEMleftUNIT"] = '';
      GASHMVKNG001db["ITEMleftVALUE"] = '';
    }
...
router.post('/GASHMVKNG001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMVKNG001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/GASHMVKNG001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = GASHMVKNG001db['confirmdata'].length
    if (all > 0) {
      GASHMVKNG001db['confirmdata'].pop();
    }
...
router.post('/GASHMVKNG001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (GASHMVKNG001db['RESULTFORMAT'] === 'Number' || GASHMVKNG001db['RESULTFORMAT'] === 'Text') {

    GASHMVKNG001db["value"] = [];
    for (i = 0; i < GASHMVKNG001db['confirmdata'].length; i++) {
      GASHMVKNG001db["value"].push({
        "PO1": GASHMVKNG001db["inspectionItemNAME"],
        "PO2": GASHMVKNG001db['confirmdata'][i]['V1'],
        "PO3": GASHMVKNG001db['confirmdata'][i]['V2'],
        "PO4": GASHMVKNG001db['confirmdata'][i]['V3'],
        "PO5": GASHMVKNG001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (GASHMVKNG001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < GASHMVKNG001db["value"].length; i++) {
        mean01.push(parseFloat(GASHMVKNG001db["value"][i]["PO3"]));
        mean02.push(parseFloat(GASHMVKNG001db["value"][i]["PO5"]));
      }
      ...
      GASHMVKNG001db["value"].push({
        "PO1": 'Mean',
        "PO2": GASHMVKNG001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": GASHMVKNG001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (GASHMVKNG001db['RESULTFORMAT'] === 'OCR' || GASHMVKNG001db['RESULTFORMAT'] === 'Picture') {

  } else if (GASHMVKNG001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < GASHMVKNG001db['confirmdata'].length; i++) {
      GASHMVKNG001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": GASHMVKNG001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (GASHMVKNG001db['RESULTFORMAT'] === 'Number' ||
    GASHMVKNG001db['RESULTFORMAT'] === 'Text' ||
    GASHMVKNG001db['RESULTFORMAT'] === 'OCR' ||
    GASHMVKNG001db['RESULTFORMAT'] === 'Picture' || GASHMVKNG001db['RESULTFORMAT'] === 'Graph') {
    request.post(
  ...
  //-------------------------------------
  return res.json(GASHMVKNG001db);
});
