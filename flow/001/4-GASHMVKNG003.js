...
router.get('/CHECK-GASHMVKNG003', async (req, res) => {

  return res.json(GASHMVKNG003db['PO']);
});


router.post('/GASHMVKNG003db', async (req, res) => {
  //-------------------------------------
  // console.log('--GASHMVKNG003db--');
  // console.log(req.body);
  //-------------------------------------
  let finddb = [{}];
  try {

    finddb = GASHMVKNG003db;
    finddbbuffer = finddb;
  }
  catch (err) {
    finddb = finddbbuffer;
...
router.post('/GETINtoGASHMVKNG003', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoGASHMVKNG003--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  check = GASHMVKNG003db;
...
router.post('/GASHMVKNG003-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < GASHMVKNG003db['ItemPickcode'].length; i++) {
    if (GASHMVKNG003db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = GASHMVKNG003db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    GASHMVKNG003db['inspectionItem'] = ITEMSS;
    GASHMVKNG003db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': GASHMVKNG003db["PO"], 'CP': GASHMVKNG003db["CP"], 'ITEMs': GASHMVKNG003db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": GASHMVKNG003db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            GASHMVKNG003db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            GASHMVKNG003db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            GASHMVKNG003db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              GASHMVKNG003db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              GASHMVKNG003db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          GASHMVKNG003db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          GASHMVKNG003db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          GASHMVKNG003db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          GASHMVKNG003db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:17180/GASHMVKNG003-feedback',
              { json: { "PO": GASHMVKNG003db['PO'], "ITEMs": GASHMVKNG003db['inspectionItem'] } },
    ...
    GASHMVKNG003db["POINTs"] = '',
      GASHMVKNG003db["PCS"] = '',
      GASHMVKNG003db["PCSleft"] = '',
      GASHMVKNG003db["UNIT"] = "",
      GASHMVKNG003db["INTERSEC"] = "",
      output = 'NOK';
  }
...
router.post('/GASHMVKNG003-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    GASHMVKNG003db['GAPnameListdata'] = graph[0];//confirmdata
    GASHMVKNG003db['GAP'] = GASHMVKNG003db['GAPnameListdata'][`GT${GASHMVKNG003db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/GASHMVKNG003-preview', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMVKNG003-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = GASHMVKNG003db['preview'][0]

    if (GASHMVKNG003db['RESULTFORMAT'] === 'Graph') {
      let pushdata = GASHMVKNG003db['preview'][0]

      pushdata['V5'] = GASHMVKNG003db['GAP'];
      pushdata['V1'] = `${GASHMVKNG003db['confirmdata'].length + 1}:${pushdata['V1']}`;

      GASHMVKNG003db['confirmdata'].push(pushdata);
      GASHMVKNG003db['preview'] = [];
      output = 'OK';
      GASHMVKNG003db['GAP'] = GASHMVKNG003db['GAPnameListdata'][`GT${GASHMVKNG003db['confirmdata'].length + 1}`]

    } else if (GASHMVKNG003db['RESULTFORMAT'] === 'Number') {

      let pushdata = GASHMVKNG003db['preview'][0]

      pushdata['V5'] = GASHMVKNG003db['confirmdata'].length + 1
      pushdata['V1'] = `${GASHMVKNG003db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/GASHMVKNG003-confirmdata-set', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003-confirmdata-set--');
  console.log(req.body);
  let input = req.body;
  GASHMVKNG003db['confirmdata'] = input;
  //-------------------------------------
  res.json('ok');
});

router.post('/GASHMVKNG003-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      GASHMVKNG003db["PCSleft"] = `${parseInt(GASHMVKNG003db["PCS"]) - oblist.length}`;
      if (GASHMVKNG003db['RESULTFORMAT'] === 'Number' || GASHMVKNG003db['RESULTFORMAT'] === 'Text' || GASHMVKNG003db['RESULTFORMAT'] === 'Graph') {
        ...
        GASHMVKNG003db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        GASHMVKNG003db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(GASHMVKNG003db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (GASHMVKNG003db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (GASHMVKNG003db['INTERSEC'] !== '') {
                core = parseFloat(GASHMVKNG003db['INTERSEC'])
              } else {
              ...
              try {
                ...
                GASHMVKNG003db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (GASHMVKNG003db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
              ...
              let dataCheck = await axios.post("http://localhost:17180/GRAPH-recal", {
                "PO": GASHMVKNG003db["PO"],
                "ITEMs": "ITEMs-5f19aaa2fe12be0020dbd3c2",
                "MODE": "CDE",
                "NAME_INS": "GAS-HMVKNG-003",
                "INTERSEC": ""
              })
            }
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:17180/JUDEMENT", { "PO": GASHMVKNG003db["PO"], "CP": GASHMVKNG003db["CP"] })
      ...
      GASHMVKNG003db["ITEMleftUNIT"] = '';
      GASHMVKNG003db["ITEMleftVALUE"] = '';
    }
...
router.post('/GASHMVKNG003-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/GASHMVKNG003-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/GASHMVKNG003-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = GASHMVKNG003db['confirmdata'].length
    if (all > 0) {
      GASHMVKNG003db['confirmdata'].pop();
    }
...
router.post('/GASHMVKNG003-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--GASHMVKNG003-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (GASHMVKNG003db['RESULTFORMAT'] === 'Number' || GASHMVKNG003db['RESULTFORMAT'] === 'Text') {

    GASHMVKNG003db["value"] = [];
    for (i = 0; i < GASHMVKNG003db['confirmdata'].length; i++) {
      GASHMVKNG003db["value"].push({
        "PO1": GASHMVKNG003db["inspectionItemNAME"],
        "PO2": GASHMVKNG003db['confirmdata'][i]['V1'],
        "PO3": GASHMVKNG003db['confirmdata'][i]['V2'],
        "PO4": GASHMVKNG003db['confirmdata'][i]['V3'],
        "PO5": GASHMVKNG003db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (GASHMVKNG003db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < GASHMVKNG003db["value"].length; i++) {
        mean01.push(parseFloat(GASHMVKNG003db["value"][i]["PO3"]));
        mean02.push(parseFloat(GASHMVKNG003db["value"][i]["PO5"]));
      }
      ...
      GASHMVKNG003db["value"].push({
        "PO1": 'Mean',
        "PO2": GASHMVKNG003db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": GASHMVKNG003db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (GASHMVKNG003db['RESULTFORMAT'] === 'OCR' || GASHMVKNG003db['RESULTFORMAT'] === 'Picture') {

  } else if (GASHMVKNG003db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < GASHMVKNG003db['confirmdata'].length; i++) {
      GASHMVKNG003db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": GASHMVKNG003db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (GASHMVKNG003db['RESULTFORMAT'] === 'Number' ||
    GASHMVKNG003db['RESULTFORMAT'] === 'Text' ||
    GASHMVKNG003db['RESULTFORMAT'] === 'OCR' ||
    GASHMVKNG003db['RESULTFORMAT'] === 'Picture' || GASHMVKNG003db['RESULTFORMAT'] === 'Graph') {
    request.post(
  ...
  //-------------------------------------
  return res.json(GASHMVKNG003db);
});
