...
router.get('/CHECK-CTCXTM001', async (req, res) => {

  return res.json(CTCXTM001db['PO']);
});


router.post('/CTCXTM001db', async (req, res) => {
  //-------------------------------------
  // console.log('--CTCXTM001db--');
...
router.post('/GETINtoCTCXTM001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoCTCXTM001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/CTCXTM001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--CTCXTM001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < CTCXTM001db['ItemPickcode'].length; i++) {
    if (CTCXTM001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = CTCXTM001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    CTCXTM001db['inspectionItem'] = ITEMSS;
    CTCXTM001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': CTCXTM001db["PO"], 'CP': CTCXTM001db["CP"], 'ITEMs': CTCXTM001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": CTCXTM001db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            CTCXTM001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            CTCXTM001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            CTCXTM001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              CTCXTM001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              CTCXTM001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          CTCXTM001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          CTCXTM001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          CTCXTM001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          CTCXTM001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:17180/CTCXTM001-feedback',
              { json: { "PO": CTCXTM001db['PO'], "ITEMs": CTCXTM001db['inspectionItem'] } },
...
router.post('/CTCXTM001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--CTCXTM001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    CTCXTM001db['GAPnameListdata'] = graph[0];//confirmdata
    CTCXTM001db['GAP'] = CTCXTM001db['GAPnameListdata'][`GT${CTCXTM001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/CTCXTM001-preview', async (req, res) => {
  //-------------------------------------
  // console.log('--CTCXTM001-preview--');
  // console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/CTCXTM001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--CTCXTM001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = CTCXTM001db['preview'][0]

    if (CTCXTM001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = CTCXTM001db['preview'][0]

      pushdata['V5'] = CTCXTM001db['GAP'];
      pushdata['V1'] = `${CTCXTM001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      CTCXTM001db['confirmdata'].push(pushdata);
      CTCXTM001db['preview'] = [];
      output = 'OK';
      CTCXTM001db['GAP'] = CTCXTM001db['GAPnameListdata'][`GT${CTCXTM001db['confirmdata'].length + 1}`]

    } else if (CTCXTM001db['RESULTFORMAT'] === 'Number') {

      let pushdata = CTCXTM001db['preview'][0]

      pushdata['V5'] = CTCXTM001db['confirmdata'].length + 1
      pushdata['V1'] = `${CTCXTM001db['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/CTCXTM001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--CTCXTM001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      CTCXTM001db["PCSleft"] = `${parseInt(CTCXTM001db["PCS"]) - oblist.length}`;
      if (CTCXTM001db['RESULTFORMAT'] === 'Number' || CTCXTM001db['RESULTFORMAT'] === 'Text' || CTCXTM001db['RESULTFORMAT'] === 'Graph') {
        ...
        CTCXTM001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        CTCXTM001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(CTCXTM001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (CTCXTM001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (CTCXTM001db['INTERSEC'] !== '') {
                core = parseFloat(CTCXTM001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                CTCXTM001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (CTCXTM001db['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:17180/JUDEMENT",{"PO":CTCXTM001db["PO"],"CP":CTCXTM001db["CP"]})
      ...
      CTCXTM001db["ITEMleftUNIT"] = '';
      CTCXTM001db["ITEMleftVALUE"] = '';
    }
...
router.post('/CTCXTM001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--CTCXTM001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/CTCXTM001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--CTCXTM001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/CTCXTM001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--CTCXTM001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = CTCXTM001db['confirmdata'].length
    if (all > 0) {
      CTCXTM001db['confirmdata'].pop();
    }
...
router.post('/CTCXTM001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--CTCXTM001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (CTCXTM001db['RESULTFORMAT'] === 'Number' || CTCXTM001db['RESULTFORMAT'] === 'Text') {

    CTCXTM001db["value"] = [];
    console.log(CTCXTM001db["value"]);
    for (i = 0; i < CTCXTM001db['confirmdata'].length; i++) {
      CTCXTM001db["value"].push({
        "PO1": CTCXTM001db["inspectionItemNAME"],
        "PO2": CTCXTM001db['confirmdata'][i]['V1'],
        "PO3": CTCXTM001db['confirmdata'][i]['V2'],
        "PO4": CTCXTM001db['confirmdata'][i]['V3'],
        "PO5": CTCXTM001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (CTCXTM001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < CTCXTM001db["value"].length; i++) {
        mean01.push(parseFloat(CTCXTM001db["value"][i]["PO3"]));
        mean02.push(parseFloat(CTCXTM001db["value"][i]["PO5"]));
      }
      ...
      CTCXTM001db["value"].push({
        "PO1": 'Mean',
        "PO2": CTCXTM001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": CTCXTM001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (CTCXTM001db['RESULTFORMAT'] === 'OCR' || CTCXTM001db['RESULTFORMAT'] === 'Picture') {

  } else if (CTCXTM001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < CTCXTM001db['confirmdata'].length; i++) {
      CTCXTM001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": CTCXTM001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (CTCXTM001db['RESULTFORMAT'] === 'Number' ||
    CTCXTM001db['RESULTFORMAT'] === 'Text' ||
    CTCXTM001db['RESULTFORMAT'] === 'OCR' ||
    CTCXTM001db['RESULTFORMAT'] === 'Picture' || CTCXTM001db['RESULTFORMAT'] === 'Graph') {
    request.post(
