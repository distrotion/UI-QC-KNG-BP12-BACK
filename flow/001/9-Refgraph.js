...
router.get('/CHECK-Refgraph', async (req, res) => {

  return res.json(Refgraphdb['PO']);
});


router.post('/Refgraphdb', async (req, res) => {
  //-------------------------------------
  // console.log('--Refgraphdb--');
...
router.post('/GETINtoRefgraph', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoRefgraph--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/Refgraph-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--Refgraph-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < Refgraphdb['ItemPickcode'].length; i++) {
    if (Refgraphdb['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = Refgraphdb['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    Refgraphdb['inspectionItem'] = ITEMSS;
    Refgraphdb['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': Refgraphdb["PO"], 'CP': Refgraphdb["CP"], 'ITEMs': Refgraphdb['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": Refgraphdb['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            Refgraphdb["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            Refgraphdb["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            Refgraphdb['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              Refgraphdb['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              Refgraphdb["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          Refgraphdb["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          Refgraphdb["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          Refgraphdb["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          Refgraphdb["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:17180/Refgraph-feedback',
              { json: { "PO": Refgraphdb['PO'], "ITEMs": Refgraphdb['inspectionItem'] } },
...
router.post('/Refgraph-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--Refgraph-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    Refgraphdb['GAPnameListdata'] = graph[0];//confirmdata
    Refgraphdb['GAP'] = Refgraphdb['GAPnameListdata'][`GT${Refgraphdb['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/Refgraph-preview', async (req, res) => {
  //-------------------------------------
  console.log('--Refgraph-preview--');
  // console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/Refgraph-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--Refgraph-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = Refgraphdb['preview'][0]

    if (Refgraphdb['RESULTFORMAT'] === 'Graph') {
      let pushdata = Refgraphdb['preview'][0]

      pushdata['V5'] = Refgraphdb['GAP'];
      pushdata['V1'] = `${Refgraphdb['confirmdata'].length + 1}:${pushdata['V1']}`;

      Refgraphdb['confirmdata'].push(pushdata);
      Refgraphdb['preview'] = [];
      output = 'OK';
      Refgraphdb['GAP'] = Refgraphdb['GAPnameListdata'][`GT${Refgraphdb['confirmdata'].length + 1}`]

    } else if (Refgraphdb['RESULTFORMAT'] === 'Number') {

      let pushdata = Refgraphdb['preview'][0]

      pushdata['V5'] = Refgraphdb['confirmdata'].length + 1
      pushdata['V1'] = `${Refgraphdb['confirmdata'].length + 1}:${pushdata['V1']}`
...
router.post('/Refgraph-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--Refgraph-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      Refgraphdb["PCSleft"] = `${parseInt(Refgraphdb["PCS"]) - oblist.length}`;
      if (Refgraphdb['RESULTFORMAT'] === 'Number' || Refgraphdb['RESULTFORMAT'] === 'Text' || Refgraphdb['RESULTFORMAT'] === 'Graph') {
        ...
        Refgraphdb["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        Refgraphdb["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(Refgraphdb["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (Refgraphdb['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (Refgraphdb['INTERSEC'] !== '') {
                core = parseFloat(Refgraphdb['INTERSEC'])
              } else {
              ...
              try {
                ...
                Refgraphdb[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (Refgraphdb['GRAPHTYPE'] == 'CDE') {
              let axis_data = [];
              for (i = 0; i < LISTbuffer.length; i++) {
                if (LISTbuffer[i]['PO1'] !== 'Mean') {
                  axis_data.push({ x: parseFloat(LISTbuffer[i].PO8), y: parseFloat(LISTbuffer[i].PO3) });
        ...
        if (CHECKlistdataFINISH.length === feedback[0]['CHECKlist'].length) {
          ...
          let dataCheck = await axios.post("http://localhost:17180/JUDEMENT",{"PO":Refgraphdb["PO"],"CP":Refgraphdb["CP"]})
      ...
      Refgraphdb["ITEMleftUNIT"] = '';
      Refgraphdb["ITEMleftVALUE"] = '';
    }
...
router.post('/Refgraph-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--RefgraphfromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/Refgraph-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--RefgraphfromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/Refgraph-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--RefgraphfromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = Refgraphdb['confirmdata'].length
    if (all > 0) {
      Refgraphdb['confirmdata'].pop();
    }
...
router.post('/Refgraph-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--Refgraph-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (Refgraphdb['RESULTFORMAT'] === 'Number' || Refgraphdb['RESULTFORMAT'] === 'Text') {

    Refgraphdb["value"] = [];
    console.log(Refgraphdb["value"]);
    for (i = 0; i < Refgraphdb['confirmdata'].length; i++) {
      Refgraphdb["value"].push({
        "PO1": Refgraphdb["inspectionItemNAME"],
        "PO2": Refgraphdb['confirmdata'][i]['V1'],
        "PO3": Refgraphdb['confirmdata'][i]['V2'],
        "PO4": Refgraphdb['confirmdata'][i]['V3'],
        "PO5": Refgraphdb['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (Refgraphdb["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < Refgraphdb["value"].length; i++) {
        mean01.push(parseFloat(Refgraphdb["value"][i]["PO3"]));
        mean02.push(parseFloat(Refgraphdb["value"][i]["PO5"]));
      }
      ...
      Refgraphdb["value"].push({
        "PO1": 'Mean',
        "PO2": Refgraphdb['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": Refgraphdb['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (Refgraphdb['RESULTFORMAT'] === 'OCR' || Refgraphdb['RESULTFORMAT'] === 'Picture') {

  } else if (Refgraphdb['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < Refgraphdb['confirmdata'].length; i++) {
      Refgraphdb["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": Refgraphdb['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (Refgraphdb['RESULTFORMAT'] === 'Number' ||
    Refgraphdb['RESULTFORMAT'] === 'Text' ||
    Refgraphdb['RESULTFORMAT'] === 'OCR' ||
    Refgraphdb['RESULTFORMAT'] === 'Picture' || Refgraphdb['RESULTFORMAT'] === 'Graph') {
    request.post(
